...
![](https://raw.githubusercontent.com/FranklinDX3906/LearnMachineLearning/master/learnMachineLearningBasic/NeuralNetworkEvolutionFormula.png)

# 课时65 模型展示2
- 将上面g括号内的式子替换为z，可以将式子替换为矩阵相乘
- 激活层->隐藏层->输出层：称之为前向传播
- 相对于计算机自己训练逻辑回归参数
- 本质上相对于把特征复杂化，多层架构可以获得更复杂的特征
...
# 课时77 随机初始化
- 使用全0作为初始参数会导致隐藏层单元的结果始终相等（相当于所有的隐藏层计算了相同的特征值）

# 课时78  组合到一起
- 隐藏层单元个数与输入层个数匹配
-  训练神经网络的步骤：1.随机初始化；2.前向传播输出y；3.计算代价函数；4.使用反向传播计算梯度，对所有样本进行前向传播和反向传播；5.检测梯度的正确性；6.优化代价函数
- 代价函数不是凸函数，可能停留在局部最优解
